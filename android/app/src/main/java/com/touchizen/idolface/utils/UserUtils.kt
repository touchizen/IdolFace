...
object UserUtils {
    ...
    private fun updateDeviceDetails(context: Context,userCollection: CollectionReference) {
        ...
        Log.d("UserUtils","AAA ${preference.getUid()}")
        Log.d("UserUtils","BB ${preference.getUserProfile()?.uId}")
        if (token.isNullOrEmpty())
            ...
            userCollection.document(preference.getUid()!!).update(updateData).addOnSuccessListener {
                ...
                Log.d("UserUtils","Token Updated $token##")
            }
        }
    }
...
//    }
    ...
    public fun logOut(context: Activity, preference: MPreference) {
        try {
