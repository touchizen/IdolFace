...
//import com.touchizen.idolface.TYPE_LOGGED_IN
...
constructor(
    ...
    fun fetchUser(taskId: Task<AuthResult>) {
        ...
        Log.d("LoginView","FetchUser:: ${user?.uid}")
        val noteRef = db.document("Users/" + user?.uid)
        noteRef.get()
            .addOnSuccessListener { data ->
                Log.d("LoginView","Uss:: ${preference.getUid()}")
                preference.setUid(user?.uid.toString())
                Log.d("LoginView","Uss11:: ${preference.getUid()}")
                preference.setLogin()
                ...
                if (data.exists()) {
                    ...
                    Log.d("LoginView","UserId ${appUser?.uId}")
